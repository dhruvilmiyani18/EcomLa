            ...
            <div id="sidebar" class="span3">
                ...
                <ul id="sideManu" class="nav nav-tabs nav-stacked">
                    ...
                    @foreach ($category_data as $cd)
                        <li class="subMenu"><a> {{ $cd->name }} </a>
                            <ul style="display:none">
                                @foreach ($subCategory_all_data as $sd)
                                    @if ($cd->id == $sd->category_id)
                                        <li><a class="active" href="{{route('product_summary',$sd->id)}}"><i class="icon-chevron-right"></i>
                                                {{ $sd->name }} 
                                            </a></li>
                                    @endif
                                @endforeach
